 ...
 */
void illum::gray_correct(cv::InputArray _src, cv::OutputArray _dst, cv::Mat average) {
  using namespace cv;
  Mat src = _src.getMat();
 ...
 *
 * @param _src the source image
 * @param _dst the destination image (must be same size as src and CV_32FC3 type)
 * @param average the appropriate average image returned by the lightfield
 * @return a new, correctly-illuminated image (in the output array's type)
 */
void illum::color_correct(cv::InputArray _src, cv::OutputArray _dst, cv::Mat average) {
  using namespace cv;
  Mat src = _src.getMat();
  ...
  if(src.size() != average.size())
    throw std::runtime_error("input image not same size as lightfield");
  Mat src32f;
  src.convertTo(src32f, CV_32FC3); // convert to floating point
  // divide image by lightfield
  Mat average32f;
  if(average.type() != CV_32FC3) {
    average.convertTo(average32f, CV_32FC3);
  } else {
    average32f = average;
  }
  // now compute corrected image in its floating-point representation
